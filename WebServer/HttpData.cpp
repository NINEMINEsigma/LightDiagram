...
const std::string html_start_file_path("index.html");
...
void HttpData::handleRead()
{
    ...
    if (!error_)
    {
        ...
        if (!error_ && state_ == ProcessState::STATE_FINISH)
        {
            ...
            if (inBuffer_.size() > 0)
            {
                if (connectionState_ != ConnectionState::H_DISCONNECTING) handleRead();
            }
...
URIState HttpData::parseURI()
{
    ...
    if (pos < 0)
    {
        fileName_ = html_start_file_path;
        HTTPVersion_ = HttpVersion::HTTP_11;
...
AnalysisState HttpData::analysisRequest()
{
    if (method_ == HttpMethod::METHOD_POST)
    {
        ...
        LOG << "HttpMethod::METHOD_POST method is not implemented";
        throw ld::LDException("HttpMethod::METHOD_POST method is not implemented");
    }
    else if (method_ == HttpMethod::METHOD_GET || method_ == HttpMethod::METHOD_HEAD)
    {
        ...
        if (method_ == HttpMethod::METHOD_HEAD) return AnalysisState::ANALYSIS_SUCCESS;
...
void HttpData::handleError(int fd, int err_num, string short_msg)
{
    ...
    body_buff += "<html><title>!!Failed!!</title>";
    body_buff += "<body bgcolor=\"ffffff\">";
    body_buff += to_string(err_num) + short_msg;
    body_buff += "<hr><em>Light Diagram Web Server</em>\n</body></html>";

    header_buff += "HTTP/1.1 " + to_string(err_num) + short_msg + "\r\n";
    header_buff += "Content-Type: text/html\r\n";
    header_buff += "Connection: Close\r\n";
    header_buff += "Content-Length: " + to_string(body_buff.size()) + "\r\n";
    header_buff += "Server: Light Diagram Web Server\r\n";
    ;
    header_buff += "\r\n";
    // 错误处理不考虑writen不完的情况
