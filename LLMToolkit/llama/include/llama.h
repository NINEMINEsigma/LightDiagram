...
namespace ld
{
    ...
    public:
        ...
        void init_instance(instance<llama_model> model, llama_context_params params)
        {
            instance_ptr = llama_new_context_with_model(model.get_ptr(), params);
            if (instance_ptr == nullptr)
            {
                throw LDException("error: failed to create the llama_context");
            }
        }
        void init_instance(instance<llama_model> model)
        {
            init_instance(model, llama_context_default_params());
        }
        // build up prompt token and init instance with default params
        instance<std::vector<llama_token>> init_instance_and_build_context(
            instance<llama_model> model,
            const string& prompt,
            int n_predict)
        {
            llama_context_params ctx_params = llama_context_default_params();
            // build up prompt token
            auto prompt_tokens = model.build_prompt_tokens(prompt);
            int n_prompt = prompt_tokens->size();
